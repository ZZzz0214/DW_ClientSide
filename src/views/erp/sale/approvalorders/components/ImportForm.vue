<template>
  <Dialog v-model="dialogVisible" title="批发销售审核导入" width="400">
    <el-upload
      ref="uploadRef"
...
defineOptions({ name: 'ErpWholesaleSaleAuditImportForm' })
...
const uploadRef = ref()
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/erp/wholesale/import-sale-audit'
const uploadHeaders = ref() // 上传 Header 头
const fileList = ref([]) // 文件列表
const updateSupport = ref(0) // 是否更新已经存在的批发销售审核数据
...
const importTemplate = async () => {
  const res = await WholesaleApi.ErpWholesaleApi.importSaleAuditTemplate()
  download.excel(res, '批发销售审核导入模板.xlsx')
}
</script>
