...
const submitFormSuccess = (response: any) => {
  ...
  let text = '导入成功数量：' + data.createCustomers.length + ';'
  for (let name of data.createCustomers) {
    text += '< ' + name + ' >'
  }
  text += '更新成功数量：' + data.updateCustomers.length + ';'
  for (const name of data.updateCustomers) {
    text += '< ' + name + ' >'
  }
  text += '更新失败数量：' + Object.keys(data.failureCustomers).length + ';'
  for (const name in data.failureCustomers) {
    text += '< ' + name + ': ' + data.failureCustomers[name] + ' >'
  }
  message.alert(text)
  formLoading.value = false
