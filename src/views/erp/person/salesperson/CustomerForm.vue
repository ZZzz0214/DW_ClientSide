      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="销售人员姓名" prop="salespersonName">
            <el-input v-model="formData.salespersonName" placeholder="请输入名称" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await ErpSalespersonApi.getSalesperson(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as SalespersonVO
    if (formType.value === 'create') {
      await ErpSalespersonApi.createSalesperson(data)
      message.success(t('common.createSuccess'))
    } else {
      await ErpSalespersonApi.updateSalesperson(data)
