<template>
  <Dialog v-model="dialogVisible" title="样品导入" width="400">
    <el-upload
      ref="uploadRef"
...
import { SampleApi } from '@/api/erp/sample'
import { getAccessToken, getTenantId } from '@/utils/auth'
import download from '@/utils/download'

defineOptions({ name: 'ErpSampleImportForm' })
...
const uploadRef = ref()
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/erp/sample/import'
const uploadHeaders = ref() // 上传 Header 头
const fileList = ref([]) // 文件列表
const updateSupport = ref(0) // 是否更新已经存在的样品数据
