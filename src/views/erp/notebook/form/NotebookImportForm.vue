<template>
  <Dialog v-model="dialogVisible" title="记事本导入" width="400">
    <el-upload
      ref="uploadRef"
...
const uploadRef = ref()
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/erp/notebook/import'
const uploadHeaders = ref() // 上传 Header 头
const fileList = ref([]) // 文件列表
const updateSupport = ref(0) // 是否更新已经存在的记事本数据
...
const importTemplate = async () => {
  const res = await NotebookApi.importNotebookTemplate()
  download.excel(res, '记事本导入模版.xlsx')
}
</script>
