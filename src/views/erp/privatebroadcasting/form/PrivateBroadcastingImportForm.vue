<template>
  <Dialog v-model="dialogVisible" title="私播货盘导入" width="400">
    <el-upload
      ref="uploadRef"
...
defineOptions({ name: 'ErpPrivateBroadcastingImportForm' })
...
const uploadRef = ref()
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/erp/private-broadcasting/import'
const uploadHeaders = ref() // 上传 Header 头
const fileList = ref([]) // 文件列表
const updateSupport = ref(0) // 是否更新已经存在的私播货盘数据
...
const importTemplate = async () => {
  const res = await ErpPrivateBroadcastingApi.importPrivateBroadcastingTemplate()
  download.excel(res, '私播货盘导入模版.xlsx')
}
</script>
