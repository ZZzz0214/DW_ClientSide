    ...
    <div class="copy-data-content" ref="copyDataRef">
      ...
      <template v-else>
        ...
        <div class="data-item">
          ...
          <span class="field-value">{{ formData.productNakedPrice || '-' }}</span>
        </div>
        <div class="data-item">
          <span class="field-name">快递费用：</span>
          <span class="field-value">{{ formData.expressFee || '-' }}</span>
        </div>
        <div class="data-item">
          <span class="field-name">代发价格：</span>
          <span class="field-value">{{ formData.dropshipPrice || '-' }}</span>
        </div>
        <div class="data-item">
          <span class="field-name">货盘状态：</span>
          <span class="field-value">{{ getStatusText(formData.privateStatus) || '-' }}</span>
        </div>
      </template>
    </div>
  </div>
...
const getStatusText = (status: string) => {
  const statusOptions = getStrDictOptions(DICT_TYPE.ERP_PRIVATE_STATUS)
...
const generateCopyText = () => {
  ...
  const lines = [
    ...
    `产品裸价：${props.formData.productNakedPrice || '-'}`,
    `快递费用：${props.formData.expressFee || '-'}`,
    `代发价格：${props.formData.dropshipPrice || '-'}`,
    `货盘状态：${getStatusText(props.formData.privateStatus) || '-'}`
  ]
  return lines.join('\n')
}
</script>
