<template>
  <Dialog v-model="dialogVisible" title="直播复盘导入" width="400">
    <el-upload
      ref="uploadRef"
...
defineOptions({ name: 'ErpLiveBroadcastingReviewImportForm' })
...
const uploadRef = ref()
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/erp/live-broadcasting-review/import'
const uploadHeaders = ref() // 上传 Header 头
const fileList = ref([]) // 文件列表
const updateSupport = ref(0) // 是否更新已经存在的直播复盘数据
...
const importTemplate = async () => {
  const res = await LiveBroadcastingReviewApi.importLiveBroadcastingReviewTemplate()
  download.excel(res, '直播复盘导入模版.xlsx')
}
</script>
